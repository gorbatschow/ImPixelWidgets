...
std::array<std::array<uint8_t, 3>, 256> turbo_srgb_bytes{
...
uint8_t ColorSchemeTurbo::red(uint8_t value) const {
  return turbo_srgb_bytes[value][0];
}

uint8_t ColorSchemeTurbo::green(uint8_t value) const {
  return turbo_srgb_bytes[value][1];
}
...
const std::array<uint8_t, 3> &ColorSchemeTurbo::rgb(uint8_t value) const {
  return turbo_srgb_bytes[value];
}
} // namespace ImPixel
